...
import {getGoods, getsearchGoods, changePage, addToFundTransferList, changePageStockGoods} from '@/action'
...
class index extends Component {
  ...
  onSelectedFormat(item, index) {
    ...
    this.iterator = this.addToFundTransferList({item, selectItem, specname})
    this.iterator.next()
  }

  * addToFundTransferList({item, selectItem, specname}) {
    let obj = {
    ...
    this.setState({num: 0, cost: 0, remark: ''})
    yield(() => {
      const next = () => {
        this.iterator.next()
      }
      const editInput = value => {
        let {HuanXingdivision,} = this.state
        let output = onVirtualKeyboard(value, this.state[HuanXingdivision], () => {
          next()
        })

        this.setState({
          [HuanXingdivision]: output
        })
      }
      const render = () => (
        <div>
          ...
          <Input
            ...
            onFocus={()=>{this.setState({HuanXingdivision: 'cost'})}}
          />
          <div>数量</div>
          <Input
            size="large"
            ...
            onFocus={()=>{this.setState({HuanXingdivision: 'num'})}}
          />
    ...
    obj['posprice'] = this.state.cost
    obj['remark'] = this.state.remark
    this.props.addToFundTransferList(obj)
  ...
  render() {
    return (
      <div className="center-custom-container" style={{flexWrap: 'wrap', paddingRight: 5}}>
        ...
        {
          this.props.stockgoods.data && this.props.stockgoods.data.data.length === 0 ?
            <h1 className="tips">此分类暂无商品</h1> : null
        }
        <Pagination
          simple
