...
import { getGoods, getoutGoods, getsearchGoods, changePage, addToputawayList, changePageStockGoods } from '@/action'
...
class index extends Component {
    ...
    onSelectedFormat(item: Object, index:Number) {
        ...
        this.iterator = this.addToputawayList({item, selectItem, specname})
        this.iterator.next()
    }

    *addToputawayList({item, selectItem, specname}) {
        // if(item.gnum <= 0){
        //     notification.open({
        //         message: '警告',
        //         description:
        ...
        this.props.addToputawayList(obj)
    ...
    onPageChange(page, pageSize) {
        ...
        if(page * pageSize > this.props.goods.data.msg.length) {
            if(this.props.setting.search==''){
                ...
                }else {this.props.getGoods({search:this.props.setting.search, page,category, store_id: this.props.store.data.id,rows:this.props.setting.rowsnum})}
            }else{
                if( path==='putaway' ){
                    this.props.getoutGoods({
                        uniacid: this.props.store.uniacid,
...
export default withRouter( connect(mapStateToProps, { getGoods,changePage,addToputawayList, getsearchGoods, changePageStockGoods, getoutGoods, })(index) )